...
def variance_norm(
    ...
    for rel_fluct_target in REL_FLUCT_TARGETS:
        ...
        relative_fluctuations[rel_fluct_target] = (variance[rel_fluct_target]
                                                   .divide(mom_ps_atom_numbers.mean().pow(2)/(ETA**2))
                                                   / (ETA**2))
        ...
        shot_noise_norm[rel_fluct_target] =  (mom_ps_atom_numbers.mean().divide(mom_ps_atom_numbers.mean().pow(2)/(ETA**2))
                                              / ETA)
        
        # Predictions for shot-to-shot fluctuations:
        sts[rel_fluct_target] = (mom_ps_atom_numbers.mean().pow(2).divide(mom_ps_atom_numbers.mean().pow(2)/(ETA**2))
                * (fluct_std_perc[rel_fluct_target]
                   / 100)**2 / (ETA**2))
        sts_error[rel_fluct_target] = (mom_ps_atom_numbers.std().pow(2).divide(mom_ps_atom_numbers.mean().pow(2)/(ETA**2))
