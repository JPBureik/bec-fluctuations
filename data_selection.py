...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr  1 14:21:29 2022
...
relative_fluctuations_error = dict.fromkeys(post_selected_data)
...
for rel_fluct_target in tqdm(post_selected_data, desc='Calculating variance'):
    ...
    relative_fluctuations_error[rel_fluct_target] = dict.fromkeys(post_selected_data[rel_fluct_target])
    ...
    for uj in post_selected_data[rel_fluct_target]:
        ...
        relative_fluctuations_error[rel_fluct_target][uj] = dict.fromkeys(post_selected_data[rel_fluct_target][uj].index.dropna())
    ...
    for uj in post_selected_data[rel_fluct_target]:
        ...
        relative_fluctuations_error[rel_fluct_target][uj] = np.std(list(bec_peak_atom_numbers[rel_fluct_target][uj].values()))/np.mean(list(shot_atom_numbers[rel_fluct_target][uj].values())) / eta(float(uj))
#%% Plot variance
...
for idx, rel_fluct_target in enumerate((0.8, 2.0, 4.0, 6.0, 9.5)):
    ...
    relative_fluctuations_error_plot = [relative_fluctuations_error[rel_fluct_target][uj] for uj in sorted(uj_vals)]
    bec_peak_atom_numbers_plot = [np.sqrt(np.mean(list(bec_peak_atom_numbers[rel_fluct_target][uj].values()))) for uj in sorted(uj_vals)]
    ...
    fluct_std_perc = 100*np.mean([np.std(list(shot_atom_numbers[rel_fluct_target][uj].values()))/np.mean(list(shot_atom_numbers[rel_fluct_target][uj].values())) for uj in sorted(uj_vals) for uj in sorted(uj_vals)])
